...
const char* const DefaultGpuProgramSkeletalAnim =
    ...
    "    float3 P = 0.0f;\n"
    "    float3 N = 0.0f;\n"
    "    if (any(input.weights0) || any(input.weights1) || any(input.weights2) || any(input.weights3) || any(input.weights4) || any(input.weights5))\n"
    "    {\n"
    "        for (int i = 0; i < 4; ++i)\n"
    "        {\n"
    ...
    "              P += mul(float4(input.position, 1.0f), boneMatrices[i + 12]).xyz * input.weights3[i];\n"
    "              P += mul(float4(input.position, 1.0f), boneMatrices[i + 16]).xyz * input.weights4[i];\n"
    "              P += mul(float4(input.position, 1.0f), boneMatrices[i + 20]).xyz * input.weights5[i];\n"
    ...
    "              N += mul(input.normal, float3x3(boneMatrices[i + 12][0].xyz, boneMatrices[i + 12][1].xyz, boneMatrices[i + 12][2].xyz)) * input.weights3[i];\n"
    "              N += mul(input.normal, float3x3(boneMatrices[i + 16][0].xyz, boneMatrices[i + 16][1].xyz, boneMatrices[i + 16][2].xyz)) * input.weights4[i];\n"
    "              N += mul(input.normal, float3x3(boneMatrices[i + 20][0].xyz, boneMatrices[i + 20][1].xyz, boneMatrices[i + 20][2].xyz)) * input.weights5[i];\n"
    "        }\n"
    "        N = normalize(N);\n"
    "    }\n"
