...
const char* const DefaultGpuProgramSkeletalAnim =
    ...
    "    if (any(input.weights))\n"
    "    {\n"
    "        matrix m = 0.0f;\n"
    "        for (int i = 0; i < 2; ++i)\n"
    "        {\n"
    "            m += boneMatrices[i] * input.weights[i];\n"
    "        }\n"
    "        P = mul(float4(P, 1.0f), m).xyz;\n"
    "    }\n"
    "    float3 posAxisConv = {P.x, P.z, -P.y};\n"
    "    output.uv = input.uv;\n"
    "    output.position = mul(float4(posAxisConv, 1.0f), worldViewProjMatrix);\n"
    "    output.colour = input.colour;\n"
    "}\n"
    "void DefaultFragmentShader(VertexOut input, out float4 output : SV_Target)\n"
    "{\n"
