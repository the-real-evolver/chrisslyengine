...
const char* const DefaultGpuProgramLit =
    ...
    "        diffuse += mul(float3(lightParams[i][0][1], lightParams[i][1][1], lightParams[i][2][1]), max(0.0f, dot(L, input.worldNormal)));\n"
    "        float specFactor = pow(max(0.0f, dot(reflect(-L, input.worldNormal), normalize(cameraPosition - input.worldPosition))), lightParams[i][3][2]);\n"
